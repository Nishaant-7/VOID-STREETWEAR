...
function handleImageFileSelect(event, prefix) {
  ...
  reader.onload = () => {
    uploadedImageData[prefix] = reader.result; 
    if (previewEl) {
      previewEl.innerHTML = `
...
async function adminAddSlide(e) {
  ...
  if (!uploadedImageData.slide) {
    alert('Please choose a banner image from your computer.');
    return;
  }
  const image = uploadedImageData.slide;

  const btnText = document.getElementById('slide-btnText').value;
...
function renderAdminSalesHistory() {
  ...
  let filtered = salesHistoryData.filter((order) => {
    let matchesMonth = monthFilter === 'all' || order.date.includes(monthFilter);
    let matchesYear = yearFilter === 'all' || order.date.includes(yearFilter);
  ...
  filtered.forEach((order, index) => {
    ...
    let statusActionCell = order.batchId
      ? `<span style="color: ${statusColor}; font-weight: bold; display: block; margin-bottom: 4px;">${order.status}</span>
         <span style="color: var(--text-secondary); font-size: 0.7rem; font-style: italic;">Managed via Lorry Batches</span>`
      : `<span style="color: ${statusColor}; font-weight: bold; display: block; margin-bottom: 4px;">${order.status}</span>
         <button class="admin-action-btn" style="background: var(--bg-primary); border: 1px solid var(--border); padding: 4px 8px; font-size: 0.75rem; color: var(--text-primary); cursor: pointer;" onclick="toggleOrderStatus(${index})" title="Change fulfillment status">
             Update Status ⟳
         </button>`;
...
function openOrderDetailModal(orderId) {
  const order = salesHistoryData.find((o) => o.orderId === orderId);
  if (!order) return;
...
function updateStatusFromOrderModal(orderId) {
  const idx = salesHistoryData.findIndex((o) => o.orderId === orderId);
  if (idx === -1) return;
...
function toggleOrderStatus(index) {
  ...
  if (currentStatus === 'Pending') {
    ...
    order.durationSec = Math.floor(Math.random() * (180 - 60 + 1)) + 60;
    order.eta = `${order.durationSec}s remaining`;

    if (!order.trackingNo) {
      order.trackingNo = 'TRK-2026-' + Math.floor(1000 + Math.random() * 9000);
    }
    ...
    if (targetEmail) {
      addNotification(
        targetEmail,
        'Package Out For Delivery',
        `Your order ${order.orderId} is now Out for Delivery! Tracking Code: ${order.trackingNo}`,
        order.trackingNo,
        {
          orderId: order.orderId,
          amount: order.amount,
...
function costAnalyticsEligibleOrders() {
  return salesHistoryData.filter((o) => typeof o.distanceKm === 'number' && o.vehicleCategory);
}
...
function renderCostAnalytics() {
  ...
  periodOrders.forEach((o) => {
    const cat = totals[o.vehicleCategory] ? o.vehicleCategory : null;
    if (!cat) return;
    const fuelL = o.fuelUsedL || 0;
    const fuelCostRM = fuelL * FUEL_PRICE_PER_LITRE_RM;
    const tollRM = o.tollCostRM || 0;
    totals[cat].deliveries += 1;
    totals[cat].distanceKm += o.distanceKm || 0;
    totals[cat].fuelL += fuelL;
    totals[cat].fuelCostRM += fuelCostRM;
    totals[cat].tollRM += tollRM;
    totals[cat].totalRM += (o.deliveryCostRM != null ? o.deliveryCostRM : fuelCostRM + tollRM);
  });
  ...
  eligible.forEach((o) => {
    ...
    const cat = buckets[key][o.vehicleCategory] ? o.vehicleCategory : null;
    if (!cat) return;
    const fuelL = o.fuelUsedL || 0;
    const fuelCostRM = fuelL * FUEL_PRICE_PER_LITRE_RM;
    const tollRM = o.tollCostRM || 0;
    ...
    b.deliveries += 1;
    b.distanceKm += o.distanceKm || 0;
    b.fuelL += fuelL;
    b.fuelCostRM += fuelCostRM;
    b.tollRM += tollRM;
    b.totalRM += (o.deliveryCostRM != null ? o.deliveryCostRM : fuelCostRM + tollRM);
  });
    ...
    [...sortedBuckets].reverse().forEach((b) => {
      Object.keys(VEHICLE_CATEGORY_META).forEach((cat) => {
        ...
        rowsHtml += `
          <tr>
            <td>${b.label}</td>
            <td><i class="fa-solid ${meta.icon}" style="color:${meta.color};"></i> ${meta.label}</td>
